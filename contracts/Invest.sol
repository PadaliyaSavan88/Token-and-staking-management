...
contract Invest is Token, Liquidity {
    ...
    modifier checkTimestamp() {
        ...
        // require(Investors[msg.sender].amount == 0 ether, "only stake holder can operate stake");
        _;
    }
    ...
    function investInMPay(uint256 _amount) public validateInvestor(_amount) {
        ...
        if(_amount > 5 ether){
            transferTokenToInvestor(msg.sender, ((_amount/ 1 ether) * 1000) + (((_amount/ 1 ether) / 20) * 1000));
        } else if (_amount > 1 ether) {
            transferTokenToInvestor(msg.sender, ((_amount/ 1 ether) * 1000) + (((_amount/ 1 ether) / 10) * 1000));
        } else {
            transferTokenToInvestor(msg.sender, (_amount/ 1 ether) * 1000);
        }
    }
    ...
    function cancelInvestment() public checkTimestamp {
        ...
        addLiquidity(stakeAmount);
        burnToken(msg.sender, balanceOf[msg.sender]);
        Investors[msg.sender] = investor(0,0,0);
    }

    function withdrawInvestment(uint256 _amount) internal {
